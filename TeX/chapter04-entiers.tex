...
\left(\begin{matrix}-9 & 4 & 11\\3 & -1 & -4\\2 & -1 & -2\end{matrix}\right)\\
  ...
  \begin{theorem}
    \label{thr:59}
    Soient $A,B \in \mathbb{Z}^{m \times m}$ en forme normale de Hermite avec $rang(A)=rang(B)=m$. Alors $\Lambda(A)=\Lambda(B) \Leftrightarrow \exists U\in \mathbb{Z}^{m \times m}$ unimodulaire telle que $B=AU$. %Doublon avec le thm 8.11 car ici U=I par notre déf de la forme normale d'hermite
  \end{theorem}
...
\begin{definition} \label{def:63}
  Soit  $A\in \mathbb{Z}^{m\times n}$ alors $\Lambda(A)=\left\{ Ax ：x\in \mathbb{R
